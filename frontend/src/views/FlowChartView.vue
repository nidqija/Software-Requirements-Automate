...
import { FlowchartFactory } from '../factory/diagramFactory';
...
const expert = FlowchartFactory.flowchart;
...
const generateDiagram = async() =>{
   ...
   try {
     const response = await fetch("http://localhost:11434/api/chat" , {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: "user", content: 'Create a flowchart for ' +  userPrompt.value }
        ],
         stream: false
      }),
     })
     ...
     const startMatch = aiReply.match(/(flowchart|graph|sequenceDiagram|classDiagram)/i);
     if (!startMatch) throw new Error("Invalid diagram format.");

    const finalCleanCode = expert.fixer(aiReply.substring(startMatch.index));
...
const saveDiagramToDB = async () => {
  ...
  try {
    ...
    formData.append("diagramType", "flowchart");
    formData.append("code", resultDiagram.value);
...
<template>
   <!------ insert html code here------> 
    <section class="home">
      ...
      <p class="subtitle">Create your flowchart diagram in minutes!</p>
      </div>
    </section>
